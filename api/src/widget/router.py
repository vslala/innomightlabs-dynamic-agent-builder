...
async def generate_image_stream(
    ...
    """Stream image generation for provider-style integrations using only the widget API key."""

    async def event_stream():
        try:
            agent = agent_repo.find_agent_by_id(api_key.agent_id, api_key.created_by)
            if not agent:
                yield SSEEvent(event_type=SSEEventType.ERROR, content="Agent not found").to_sse()
                return
            ...
            log.error("Error in widget image generation stream: %s", e, exc_info=True)
            yield SSEEvent(event_type=SSEEventType.ERROR, content=str(e)).to_sse()
